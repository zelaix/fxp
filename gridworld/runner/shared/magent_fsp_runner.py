...
class MAgentFSPRunner(Runner):

    def __init__(self, config):
        super(MAgentFSPRunner, self).__init__(config)
        self.update_sp = dict(update_sp=True)
        self.fp_interval = self.all_args.fp_interval
        self.num_oppo_policy = 0
        self.old_env_infos = defaultdict(list)

    def run(self):
        self.save()
        self.envs.update([self.update_sp for _ in range(self.n_rollout_threads)])

        self.save_model()
        oppo_sample_probs = np.ones(self.num_oppo_policy) / self.num_oppo_policy
        update_kwargs = dict(update_population=True, oppo_sample_probs=oppo_sample_probs)
        ...
        for episode in range(episodes):
            ...
            if total_num_steps % self.fp_interval == 0:
                self.save_model()
                # update env opponents and probs
                oppo_sample_probs = np.ones(self.num_oppo_policy) / self.num_oppo_policy
                update_kwargs = dict(update_population=True, oppo_sample_probs=oppo_sample_probs)
                self.envs.update([update_kwargs for _ in range(self.n_rollout_threads)])
                self.warmup()
            ...
            if total_num_steps % self.log_interval == 0:
                ...
                sp_idx = np.array(self.env_infos["sp_prob"])
                if np.sum(sp_idx) == 0:
                    self.env_infos[f"win_rate_sp"] = self.old_env_infos[f"win_rate_sp"]
                    self.env_infos[f"draw_rate_sp"] = self.old_env_infos[f"draw_rate_sp"]
                    self.env_infos[f"lose_rate_sp"] = self.old_env_infos[f"lose_rate_sp"]
                else:                        
                    self.env_infos[f"win_rate_sp"] = np.array(self.env_infos["left_win_rate"])[sp_idx]
                    self.env_infos[f"draw_rate_sp"] = np.array(self.env_infos["draw_rate"])[sp_idx]
                    self.env_infos[f"lose_rate_sp"] = np.array(self.env_infos["right_win_rate"])[sp_idx]
