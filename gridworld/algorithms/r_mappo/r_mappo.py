...
from gridworld.utils.util import get_gard_norm, huber_loss, mse_loss
from gridworld.utils.valuenorm import ValueNorm
from gridworld.algorithms.utils.util import check
...
class R_MAPPO():
    def __init__(self,
        ...
        self._use_neurd = args.use_neurd
    ...
    def ppo_update(self, sample, turn_on=True):
        ...
        values, action_log_probs, dist_entropy, policy_values = self.policy.evaluate_actions(share_obs_batch,
                                                                              ...
                                                                              active_masks_batch)
        # actor update
        if not self._use_neurd:
            ratio = torch.exp(action_log_probs - old_action_log_probs_batch)
        else:
            ratio = action_log_probs / (old_action_log_probs_batch + 1e-5)
