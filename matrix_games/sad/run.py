...
from matrix_games.sad.game import SADGame
...
class EvalLogger(object):
    def __init__(self, game):
        ...
        self.T, self.q = 0, []
        self.log_mode = 3 # log gain_expl, attack_expl, defend_expl

        self.end_point = []
    
    def eval(self, pi):
        N, A = self.N, self.A
        gain_pi = np.eye(A + 3)[np.repeat(A, N)]
        attack_pi = np.copy(gain_pi)
        attack_pi[:game.attack_force, :] = np.eye(A + 3)[[A + 1] * game.attack_force]
        defend_pi = np.copy(gain_pi)
        defend_pi[0] = np.eye(A + 3)[A + 2]

        gain_expl = game.get_value(np.concatenate([gain_pi, pi], 0))
        attack_expl = game.get_value(np.concatenate([attack_pi, pi], 0))
        defend_expl = game.get_value(np.concatenate([defend_pi, pi], 0))
        return gain_expl, attack_expl, defend_expl
    
    def log(self, pi):
        gain_expl, attack_expl, defend_expl = self.eval(pi)
        if self.log_mode == 2:
            self.q.append(np.array([max(0, gain_expl), max(0, attack_expl)]))
        elif self.log_mode == 3:
            self.q.append(np.array([max(0, gain_expl), max(0, attack_expl), max(0, defend_expl)]))
        else:
            raise NotImplementedError
        self.T += 1
    ...
    def batch_eval(self, pi_list):
        ...
        expl_list = []
        for expl_policy in [gain_pi, attack_pi, defend_pi]:
            eval_list = []
            for pi in pi_list:
        ...
        return expl_list[0], expl_list[1], expl_list[2]
    
    def log_meta(self, meta_pi):
        pi_list, coeff = meta_pi
        gain_expl, attack_expl, defend_expl = self.batch_eval(pi_list)
        gain_expl = (gain_expl * coeff).sum()
        attack_expl = (attack_expl * coeff).sum()
        defend_expl = (defend_expl * coeff).sum()
        if self.log_mode == 2:
            self.q.append(np.array([max(0, gain_expl), max(0, attack_expl)]))
        elif self.log_mode == 3:
            self.q.append(np.array([max(0, gain_expl), max(0, attack_expl), max(0, defend_expl)]))
...
if __name__ == "__main__":
    ...
    # to_run = ["FXP_reset", "FXP_reset_c", "FXP_reset_meta"] #, "FXP_c", "FXP_meta", "PSRO_uniform", "PSRO_uniform_meta", "PSRO_uniform_warm", "PSRO_uniform_warm_meta", "PSRO_NE", "PSRO_NE_meta", "PSRO_NE_warm", "PSRO_NE_warm_meta"]
    np.random.seed(0)

    N, A, C = 4, 10, 0
    game = SADGame(N, A, episode_length=1, attack_cost=C, attack_force=2)
    pi_0 = get_pi0(N, A + 3, pi0_gen_mode)

    nCol, nRow = 5, max(2, (len(to_run) + 2) // 5)
    ...
    red_patch = mpatches.Patch(color="red", label="expl gain")
    green_patch = mpatches.Patch(color="green", label="expl attack")
    blue_patch = mpatches.Patch(color="blue", label="expl defend")
    fig.legend(handles=[red_patch, green_patch, blue_patch], fontsize=30)
    for i, name in enumerate(to_run):
        axes[i // nCol, i % nCol].set_title(name, fontsize=30)
    fig.savefig("figs/sad_all.png")
